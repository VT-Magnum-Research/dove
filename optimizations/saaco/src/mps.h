...
private:
  ...
  // Allows us to know if a local search has been performed on the current tour
  bool local_search_occurred_;
...
public:
  ...
  std::vector<unsigned int>& get_mapping_from_task_to_core() {
    return mapping_;
  }
  
  int get_routing_cost(int from, int to)  {
    return routing_costs_->operator[](from)[to];
  }
  
  void set_ant_colony(AntColony<Ant>* colony);
  
  void print_mapping(std::vector<unsigned int> tour, std::ostream &out);
  
  void print_detailed_schedule(std::vector<unsigned int> tour, std::ostream &out);
