...
private:
  ...
  Matrix<unsigned int>* run_times_;
  DirectedAcyclicGraph* task_precedence_;
  
public:
  MpSchedule(SymmetricMatrix<unsigned int>* routing_costs,
  ...
  unsigned int get_earliest_start_time(Task* t, unsigned int core_id, std::vector<std::string>& delays, bool maintain_routing_time = true);
